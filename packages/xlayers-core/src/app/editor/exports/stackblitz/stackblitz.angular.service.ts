  ...
  prepare(content: XlayersNgxEditorModel[]): StackBlitzProjectPayload {
    ...
    for (let i = 0; i < content.length; i++) {
      for (const prop in content[i]) {
        if (prop === 'uri') {
          files[`packages/xlayers-core/src/app/xlayers/` + content[i].uri] = content[i].value;
        }
      }
    }

    files['packages/xlayers-core/src/app/app.component.ts'] = `
...
export class AppComponent  {}
    `;
    files['packages/xlayers-core/src/app/app.module.ts'] = `
